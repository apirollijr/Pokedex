    ...
    return Promise.all(
      types.map((type) =>
        fetch(type.url)
          ...
          .then((details) => ({
            name: type.name,
            doubleTo: details.damage_relations.double_damage_to.map(
              (t) => t.name
            ),
            doubleFrom: details.damage_relations.double_damage_from.map(
              (t) => t.name
            ),
            halfFrom: details.damage_relations.half_damage_from.map(
              (t) => t.name
            ),
