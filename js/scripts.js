...
const pokemonRepository = (function () {
  ...
  function loadDetails(pokemon) {
    ...
    return fetch(pokemon.detailsUrl)
      ...
      .then((details) => {
        ...
        pokemon.baseStats = details.stats.map((s) => ({
          name: s.stat.name,
          value: s.base_stat,
        }));
  ...
  function loadNextChunk() {
    ...
    return Promise.all(chunk.map((pokemon) => loadDetails(pokemon))).then(
      () => chunk
    );
  }
...
function createStatBars(stats) {
  ...
  stats.forEach((stat) => {
    ...
    label.innerHTML = `<span>${stat.name.toUpperCase()}</span><span>${
      stat.value
    }</span>`;
...
function filterPokemon(type) {
  ...
  const filtered =
    type === "all" ? all : all.filter((p) => p.types && p.types.includes(type));
